        ...
        <PortfolioContext.Consumer>
            ...
            {state.isModalOpen && (
                 <div className="project-modal">
                    <div className="project-modal__exit" onClick={e => {toggleModal(currentProjectNum)}}>
                    ...
                    <div className="project-modal__toggle-backwards" onClick={toggleProjectsForward}>
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                            <path  d="M13.025 1l-2.847 2.828 6.176 6.176h-16.354v3.992h16.354l-6.176 6.176 2.847 2.828 10.975-11z"/>
                        </svg>
                    </div>
                    <div className="project-modal__toggle-forwards" onClick={toggleProjectsBackward}>
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                            <path d="M11,23l2.8-2.8L7.6,14H24v-4H7.6l6.2-6.2L11,1L0,12L11,23z"/>
                        </svg>
                    </div>
                    <div className="content">
                        <h2 className="fade-in" style={{color: '#ffffff', marginTop: '40px', clear: "both", userSelect: 'none'}}>
                            {portfolio[currentProjectNum].project_name}
                        </h2>
                        <h3 className="fade-in" style={{color: '#ffffff', clear: "both", lineHeight: '0', paddingBottom: '40px', userSelect: 'none'}}>
                            {portfolio[currentProjectNum].date}
                        </h3>
                        <div className="fade-in featured-image" style={document.documentElement.classList.contains('webp') ? {backgroundImage: 'url(' + portfolio[currentProjectNum].image.webp + ')'} : {backgroundImage: 'url(' + portfolio[currentProjectNum].image.fallback + ')'}}></div>
                        <p className="fade-in" style={{color: '#ffffff', clear: "both", padding: '40px 0px 20px 0px'}}>
                            {portfolio[currentProjectNum].description}
                        </p>
                        <img src={document.documentElement.classList.contains('webp') ? portfolio[currentProjectNum].image.webp : portfolio[currentProjectNum].image.fallback} className="fade-in" />
                        {
